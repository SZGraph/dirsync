...
### 1. по имени;
### 2. по размеру файла;
### 3. по MD5 хешу в случае равных размеров;
### 4. по дате создания.
...
### Второй приоритет у размера файла. Если файлы равны, то они сравниваются по MD5 хешу (актуально для случаев где в файле изменен параметр, цифра). Если содержания файла ИСТОЧНИКА и ПРИЕМНИКА идентичны, то копирование не произойдет.
### Если файлы не равны по размеру или MD5 хешу, то учитывается время создания файлов.
...
### Последняя модификация: 20.01.2023 г., Сергей Злобин
#############################################################################################################################################
...
# К дескрипторам ввода-вывода добавляется кодировка из текущей локали ($ENV{LANG} = 'ru_RU.KOI8-R';), и такие операции, как chr и ord, используют эту кодировку.
# Имеет смысл только в том случае, если сам скрипт написан в ASCII, а все возможные локали, которые будут использоваться при 
# выполнении скрипта, являются надмножествами ASCII.
use open ':locale';
...
# Список регулярных выражений не синхронизируемых файлов, директорий и ссылок.
...
# <regex> будет применено к каждому имени файла из директории ИСТОЧНИКА (как к отдельным строкам).
...
# Для копирования используется функция rsync от Andrew Tridgell, Wayne Davison, and others. rsync прдустановлена на многие дистрибутивы linux. 
...
#--archive, -a	archive mode (сохраняет время изменения, права пользователей и группы);
#--checksum, -c	опция изменяет способ, которым rsync проверяет, были ли файлы изменены и нуждаются в передаче.  Без этой опции rsync использует "быструю проверку", 
# 				которая (по умолчанию) проверяет, совпадают ли размер каждого файла и время последней модификации совпадают у отправителя и получателя.
#--preallocate	Это указывает приемнику выделить каждому файлу назначения его возможный размер перед записью данных в файл.  Rsync будет только использовать 
#				реальную поддержку предварительного выделения на уровне файловой системы, предоставляемую системным вызовом fallocate(2) в Linux или posix_fallocate(3) в Cygwin, 
#				а не медленную реализацию glibc, которая записывает нулевой байт в каждый блок.
#				Без этой опции большие файлы могут быть не полностью смежными в файловой системе, но с этой опцией rsync, вероятно, будет копировать медленнее.   
#				Если местом назначения не является файловая система с поддержкой экстентов (например, ext4, xfs, NTFS и т.д.), эта опция может вообще не иметь положительного эффекта.
my $file_copy_sub = sub ($$) {my ($i_s_source_file, $i_s_dest_file)=@_;# Инициализация входных параметров функций
	qx(rsync -ac --preallocate '$i_s_source_file' '$i_s_dest_file' 2>&1 1>/dev/null)# Вывод STDOUT и STDERR перенаправляется в /dev/null
	#qx(zip - '$i_s_source_file'|zcat > '$i_s_dest_file');
	};
#Вызов функции: &$file_copy_sub('/media/.../Coding_theory/ReedMullerExamples.pdf','/media/.../Coding_theory_copy/ReedMullerExamples.pdf');

# Функция копирования директории
# Для копирования используется функция rsync.
# Особенности синтаксиса rsync:
# rsync [опции] ИСТОЧНИК ПРИЕМНИК
#	rsync [опции] src/bar /data/tmp в Приемнике появится /data/tmp/bar чтобы избежать создания дополнительного уровня каталога в месте назначения необходимо использовать "/" в конце пути ИСТОЧНИКА.
#	rsync [опции] src/bar/ /data/tmp в ПРИЕМНИКЕ в /data/tmp/ будет содержимое src/bar/
my $dir_copy_sub = sub ($$) {my ($i_s_source_dir, $i_s_dest_dir)=@_;# Инициализация входных параметров функций
	$i_s_source_dir .= "/"; # конкатенировали "/" чтобы избежать создания дополнительного уровня каталога
	qx(rsync -ac --preallocate '$i_s_source_dir' '$i_s_dest_dir' 2>&1 1>/dev/null)# Вывод STDOUT и STDERR перенаправляется в /dev/null
};
...
# Функция создания директории в глубину — mkdir_r(…).
#----------------------------------------------------------------------------------------------------------------------------- 
# Алгоритм: 
...
sub RUN($;$)
{
	...
	my $s_read_files = join("|",@a_read_files);
	$s_read_files = "|".$s_read_files."|"; # обрамляем все элементы разделителем "|"
	...
	{
		# Удаляем каждый мач вида:'|<regex>'
		$s_read_files =~ s/\|$s_unit_nonsinchr(?=\|)//g;
	}
	
	# Удаляем лишние разделители "|" если есть
	$s_read_files =~ s/\|+/\n/g;
	$s_read_files =~ s/^\n//g;
				...
				if (-e $s_dest_element)
				{
					...
					if ($s_result_compare == 1)
					{
						...
						if (!-e "$s_dest_dir/$PARAM_s_archive_dir_name")
						{
							...
							if (mkdir_r("$s_dest_dir/$PARAM_s_archive_dir_name") != 1){print "Директория не создана: $s_dest_dir/$PARAM_s_archive_dir_name";};
	...
	else
	{
		...
		if (mkdir_r($s_dest_dir) != 1){print "Директория не создана: $s_dest_dir";};
...
sub FileComparator($$)
{
	...
	# Задание переменных для хранения контрольной суммы (MD5 хеша) сравниваемых файлов
	my $s_digest_file1 = '';
	my $s_digest_file2 = '';
	...
	if (-s $i_s_file1 == -s $i_s_file2)
	{
		...
		# Проверку различия осуществим по MD5 хешу. Если хеши равны, то и файлы равны
		
		# Получение MD5 хешей файлов
		# -b		read in binary mode
		# --tag		create a BSD-style checksum: "MD5 (</your/file>) = bd606d43e04bf3a81cb4106e7f0caa63"
		# регулярное выражение "\) = (.+)" мачит контрольную сумму
		($s_digest_file1) = qx(md5sum -b --tag '$i_s_file1') =~ m/\) = (.+)/; #file_md5_hex($i_s_file1);
		($s_digest_file2) = qx(md5sum -b --tag '$i_s_file2') =~ m/\) = (.+)/; #file_md5_hex($i_s_file2);
		
		# Проверка, равны ли хеши файлов
		if ($s_digest_file1 eq $s_digest_file2)
		{
			
			# хеши файлов равны => файлы не различаются
			$o_s_result_compare = 0;
		}
	}
...
#=================================================================
# IV. Функция создания директории в глубину - mkdir_r(...). 
#=================================================================
# Возвращает 0 - если НЕ создана, 1 - иначе
sub mkdir_r($)
{
	# Получаем на вход директорию
	my ($i_s_path) = @_;
	my $s_this_dir = '';
	$i_s_path .= '/';
	# делим путь на части и проходим по нему
	my @a_dirs = ($i_s_path =~ m/(.+?)(?=\/)/g);

	foreach my $s_dir (@a_dirs)
	{
		# $s_dir - содержит директорию в виде /xxx
		$s_this_dir .= $s_dir; # конкатинируем с предыдущими
		mkdir "$s_this_dir" if ((!-d "$s_this_dir") and (!-f "$s_this_dir"));# создаем если нет
	}
	$i_s_path =~ s/<$//;
	# проверяем создана или нет
	if (-d "$i_s_path")
	{
		# создана. Возвращаем 1
		return 1
	}
	else
	{
		# НЕ создана. Возвращаем 0
		return 0
	}
}

#=================================================================
# V. Основная программа сравнения файлов
#=================================================================
...
print <<Report
\tОтчет от $nice_timestamp о синхронизации директории ИСТОЧНИКА
